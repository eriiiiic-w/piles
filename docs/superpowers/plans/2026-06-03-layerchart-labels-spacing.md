# LayerChart 标签修复 + 尺寸间距优化

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 修复剖面图顶标高标签不显示问题，增大图表尺寸，缩紧柱体间距。

**Architecture:** 仅修改 LayerChart.tsx（图表组件）和 PredictPage.tsx（包裹 Card 的宽度），无新文件、无后端改动。

**Tech Stack:** React, TypeScript, Recharts

---

### Task 1: 修复顶标高标签 + 增大图表 + 缩紧间距

**Files:**
- Modify: `client/src/components/charts/LayerChart.tsx`
- Modify: `client/src/pages/PredictPage.tsx:115-124` (Card 宽度)

- [ ] **Step 1: 修改 LayerChart.tsx — 标签改用 `<Bar label>` + 增大高度 + 缩紧间距**

将 `LayerChart.tsx` 中的图表配置做以下修改：

1. `height` 420 → 550
2. 添加 `barCategoryGap={3}` `barGap={2}` 到 `<BarChart>`
3. 预测土层 `<Bar>` 的 `<LabelList>` 改为 `label` prop
4. 实测土层也添加类似的 label（与原始程序一致，实测顶标高也要显示标签）

修改后的完整 `LayerChart.tsx`：
      ...
      <BarChart data={data} margin={{ top: 30, right: 30, left: 20, bottom: 5 }}
        ...
        <Bar dataKey="预测厚度" stackId="pred" name="预测土层"
          label={({ x, y, width, value, index }: any) => {
            ...
            if (top <= 0 && top > -999) return (
              <text x={x + width / 2} y={y + 14} textAnchor="middle" fontSize={10} fill="#2980b9" fontWeight={600}>
                {top.toFixed(2)}
              </text>
            );
            return (
              <text x={x + width / 2} y={y - 4} textAnchor="middle" fontSize={10} fill="#2980b9" fontWeight={600}>
                {top.toFixed(2)}
              </text>
            );
          }}
        ...
        <Bar dataKey="实测厚度" stackId="meas" fill="#e74c3c" name="实测土层"
          label={({ x, y, width, value, index }: any) => {
            ...
            const top = base + (typeof value === 'number' ? value : 0);
            return (
              <text x={x + width / 2} y={y - 4} textAnchor="middle" fontSize={10} fill="#c0392b" fontWeight={600}>
                {top.toFixed(2)}
              </text>
            );
          }}
...
- [ ] **Step 2: 修改 PredictPage.tsx — Card 去掉宽度限制**

当前 Card 没有显式宽度限制，保持不变即可。如果 Card 被父容器限制了宽度，可以添加 `style` 扩展。
...
Expected: TypeScript 零错误，构建成功。

- [ ] **Step 4: Commit**

```bash
git add client/src/components/charts/LayerChart.tsx
git commit -m "fix: LayerChart — 标签改用Bar label修复显示 + 增大图表 + 缩紧间距"
```
